...
public class CreateDots : MonoBehaviour {
    ...
    public string barType = "prefabs/lines/prefabDot"; //prefab name: can be bar, dot, ...

    void Start () {

        Color catColor = new Color(100, 0, 0);
        ...
        for (int numCat = 0; numCat < JSONtoObj.MainChart.category.Count; numCat++)  // per category...
        {
            ...
            catColor = Random.ColorHSV(0f, 1f, 1f, 1f, 0.5f, 1f);
            ...
            for (int numValues = 0; numValues < JSONtoObj.MainChart.xaxis.values.Count; numValues++)  // ... create the row of bars
            {
                ...
                GameObject bar = Instantiate(Resources.Load(barType), new Vector3(numValues * barSeparation, (float)height, 0), Quaternion.identity) as GameObject;
                bar.GetComponent<Renderer>().material.color = catColor;
