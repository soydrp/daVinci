...
public class DataPoint // each data point can have three values and a commentary - what you do with each value is up to the chart logic
{

    public string xlvl1 { get; set; }
    public string xlvl2 { get; set; }
    public string zlvl1 { get; set; }
    public string zlvl2 { get; set; }
...
public class JSONtoObj_5D : MonoBehaviour {
    ...
    DataPoint readDataPointJson(string JSONstr)
    {
        DataPoint datPoint = new DataPoint();
        JSONObject jsonObject = JSONObject.Parse(JSONstr);
    ...
    void Awake() {  //Awake is called before Start()
        ...
        foreach (KeyValuePair<string, JSONValue> pair in jsonObject)
        {
            ...
            if (pair.Key == "XAXISLevel2") { MainChart.xaxis = readXAxisJson(pair.Value.ToString()); }
            if (pair.Key == "ZAXISLevel2") { MainChart.yaxis = readYAxisJson(pair.Value.ToString()); }
           // if (pair.Key.Contains("YAXIS")) { MainChart.category.Add(readCategoryJson(pair.Value.ToString())); }
        }
    }
